...
Файл появится в `screenshots/telegram-delivery-settings/<timestamp>.png`. Для страницы Analytics: из `web/` выполнить `npm run screenshot:analytics` (нужны API и frontend) — скриншот в `screenshots/analytics/<timestamp>.png`. Для Streams: `npm run screenshot:streams` — скриншот в `screenshots/streams/<timestamp>.png`. Для Stream Player (/streams/:id): `npm run screenshot:stream-detail` — скриншот в `screenshots/stream-detail/<timestamp>.png`. Ничего вручную не запускать, не использовать MCP browser для логина — всё делает контейнер. Если Docker недоступен — см. ниже «Снятие скриншота (MCP browser)» или «npm run screenshot:settings» (нужны запущенные API и frontend).
