...
### Decision
Worker выполняет периодический cleanup: удаляет результаты проверок старше TTL (по умолчанию 30 дней) и соответствующие файлы скриншотов в FS. API не выполняет retention-задачи и не удаляет данные. Cleanup выполняется батчами по компании (tenant scope: все запросы по `company_id`); путь к файлам — `storage/{company_id}/{stream_id}/{check_id}.jpg` (см. ADR-0003). Конфиг: `RETENTION_TTL_DAYS`, `RETENTION_CLEANUP_INTERVAL_MIN`, `RETENTION_CLEANUP_BATCH_SIZE` (см. `.env.example` и **`docs/retention_cleanup.md`**).
...
- Tenant-границы при cleanup соблюдаются; идемпотентность повторного запуска сохраняется.
...
- cooldown: после отправки молчать T минут (по stream_id)
- recovered (опционально): при переходе FAIL→OK отправлять "recovered" с тем же cooldown

Детальный контракт алертов (переходы статуса, формат сообщения, антиспам): **`docs/telegram_alerts_contract.md`**.
