...
|---------------|-------------|------------------------------------|
| OK → WARN     | (планируется) | Первое предупреждение по потоку   |
...
- **OK→WARN и WARN→FAIL (первый переход):** в коде могут быть добавлены по той же модели (cooldown по stream_id).
...
- **Event** — тип события: `FAIL`, `RECOVERED` (и при добавлении — `WARN`).
