#!/usr/bin/env node
/**
 * Automated screenshot of /streams (authorized). Saves to screenshots/streams/<timestamp>.png.
...
const SCREENSHOTS_DIR = path.join(ROOT_DIR, "screenshots", "streams");
const REPORT_PATH = path.join(ROOT_DIR, "screenshots", "streams", "REPORT.txt");
...
async function main() {
  log("FE-STREAMS-001: streams page screenshot");
  ...
  try {
    ...
    const hasTitle = await page.locator("h2.page-title:has-text('Streams')").count() > 0;
    const hasSearch = await page.locator("#streams-search").count() > 0;
    const hasProjectFilter = await page.locator("#streams-project-filter").count() > 0;
    const hasActiveFilter = await page.locator("#streams-active-filter").count() > 0;
    const hasStatusFilter = await page.locator("#streams-status-filter").count() > 0;
    const hasTableOrEmpty =
      (await page.locator(".table-wrap table").count()) > 0 ||
      (await page.locator("text=No streams found").count()) > 0 ||
    ...
    let score = 9;
    if (!hasTitle) score = 7;
    if (!hasSearch || !hasProjectFilter || !hasStatusFilter) score = Math.min(score, 8);
    if (!hasTableOrEmpty) score = Math.min(score, 8);

    const reportLines = [
      "FE-STREAMS-001 REPORT",
      "Screenshot: " + path.relative(ROOT_DIR, screenshotPath),
      "Score: " + score,
      "Checks: authorized /streams, Streams title, Search/Project/Active/Status filters, table or empty state.",
    ];
